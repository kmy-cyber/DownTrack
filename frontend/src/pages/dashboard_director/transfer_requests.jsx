...
const TransferRequestsTable = () => {
    ...
    const fetchTransferRequests = async (pageNumber) => {
        ...
        try {
            ...
            const response = await api(`/TransferRequest/GetPaged?PageNumber=${pageNumber}&PageSize=${pageSize}`, {
                method: 'GET',
            });
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full table-auto text-sm text-gray-900 border-collapse">
                                <thead className="bg-gray-800 text-white">
                                    <tr>
                                        <th className="px-6 py-3 border-b text-center">Requester</th>
                                        <th className="px-6 py-3 border-b text-center">Request ID</th>
                                        <th className="px-6 py-3 border-b text-center">Equipment</th>
                                        <th className="px-6 py-3 border-b text-center">Transfer Date</th>
                                    </tr>
                                </thead>
                                <tbody className="bg-white">
                                    {transferRequestsList.length > 0 ? (
                                        transferRequestsList.map((request) => (
                                            <tr key={request.id}>
                                                <td className="px-6 py-3 border-b text-center">{request.sectionManagerUserName}</td>
                                                <td className="px-6 py-3 border-b text-center">{request.arrivalSectionName}</td>
                                                <td className="px-6 py-3 border-b text-center">{request.arrivalDepartmentName}</td>
                                                <td className="px-6 py-3 border-b text-center">{request.date}</td>
                                            </tr>
                                        ))
                                    ) : (
                                        <tr>
                                            <td colSpan="6" className="px-6 py-3 text-center">No transfer requests found</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
