...
const EvaluationsTable = () => {
    ...
    const fetchEvaluations = async (pageNumber) => {
        ...
        try {
            ...
            const response = await api(`/Evaluation/GetPaged?PageNumber=${pageNumber}&PageSize=${pageSize}`, {
                method: 'GET',
            });
                            ...
                            <table className="min-w-full table-auto text-sm text-gray-900 border-collapse">
                                <thead className="bg-gray-800 text-white">
                                    <tr>
                                        ...
                                        <th className="px-6 py-3 border-b text-center">Evaluator</th>
                                        <th className="px-6 py-3 border-b text-center">Evaluation</th>
                                    </tr>
                                </thead>
                                    ...
                                    {evaluationsList.length > 0 ? (
                                        evaluationsList.map((evaluation) => (
                                            <tr key={evaluation.id}>
                                                <td className="px-6 py-3 border-b text-center">{evaluation.technicianUserName}</td>
                                                <td className="px-6 py-3 border-b text-center">{evaluation.sectionManagerUserName}</td>
                                                <td className="px-6 py-3 border-b text-center">{evaluation.description}</td>
                                            </tr>
                                        ))
                                    ) : (
                                        <tr>
                                            <td colSpan="4" className="px-6 py-3 text-center">No evaluations found</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
